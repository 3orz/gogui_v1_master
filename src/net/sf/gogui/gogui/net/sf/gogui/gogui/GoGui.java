...
{ 
        ...
        throws GtpError, ErrorMessage
    {
        ...
        m_infoPanel = new BgPanel();
        m_infoPanel.setLayout(new BorderLayout());
        m_infoPanel.setPreferredSize(new Dimension(400, 700));
        m_infoPanel.setMinimumSize(new Dimension(30, 70));
        m_infoPanel.setMaximumSize(new Dimension(300, 700));
        m_game = new Game(boardSize);


        m_gameInfoPanel = new GameInfoPanel(m_game);
        m_gameInfoPanel.setOpaque(false);
        //m_gameInfoPanel.setOpaque(false);
        m_gameInfoPanel.setBorder(GuiUtil.createSmallEmptyBorder());
        //abc.add(m_gameInfoPanel);

        //*****HERE********
        //m_infoPanel.add(m_gameInfoPanel);//, BorderLayout.CENTER);
/*        JPanel paaa = new JPanel();
        
        JTextField asd = new JTextField(10);
        asd.setBackground(new Color(250,250,250,0));
        paaa.add(asd);
        m_infoPanel.add(paaa,BorderLayout.SOUTH);*/

        m_infoPanel.add(m_gameInfoPanel, BorderLayout.NORTH);
//paaa.setOpaque(false);
...
//////////////////////////////////////////////////////////
    ...
    JFrame frame = new JFrame("GoGui Fullscreen");

// ------ insert 21.06

    int shadow = 0;
    int m_backgroundNum = 0;
...
// ------ insert 28.07
	...
	boolean lock = false;
	boolean play = false;
    boolean kgsMode = false;
    boolean remoteMode = false;


    public void actionKgsServer()
    {
       
	    KgsGtpEngine fuuu = new KgsGtpEngine();
        
    }

    public void actionIgs(){
    ...
    public void actionResign(){


        if(igsMode == true){
            m_gameInfoPanel.move("resign");
            play = false;
            unprotectGui();
        }

        String mainMessage = format(i18n("MSG_RESIGN"), "You");
                            String optionalMessage =
                            "You has resigned the game.";
...
//// ----------- insert date 17.06.2016
//// --date 27.06 background

    public void actionSetBackground(String name){
    m_guiBoard.changeImageBackground(name);
    m_guiBoard.repaintPanel();
    }

    public void actionExitFullscreenMode()
    {
    ...
    public void actionFullscreenMode()
    {
        frame = new JFrame("GoGui Fullscreen");
        ...
        GuiAction.registerAll(m_guiBoard);
        m_guiBoard.repaintPanel();
    
// -------- insert debug fullscreen 20.06

        Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();
        m_guiBoard.setPreferredSize(screenSize);

// -------- insert end
    }

   public void actionSetShadow()
   {
    if(shadow == 0)
        shadow = 1;
    else
        shadow = 0;
    actionChangeBackground(m_backgroundNum);
   }

   public void actionChangeBackground(int backgroundNum)
   {
    m_backgroundNum = backgroundNum;

    if (m_backgroundNum == 0)
        m_guiBoard.setBackground(0,shadow);

    else if (m_backgroundNum == 1)
        m_guiBoard.setBackground(1,shadow);

    else if (m_backgroundNum == 2)
        m_guiBoard.setBackground(2,shadow);

    else if (m_backgroundNum == 3)
        m_guiBoard.setBackground(3,shadow);

    else if (m_backgroundNum == 4)
        m_guiBoard.setBackground(4,shadow);

    else if (m_backgroundNum == 5)
        m_guiBoard.setBackground(5,shadow);

    else if (m_backgroundNum == 6)
        m_guiBoard.setBackground(6,shadow);

    else if (m_backgroundNum == 7)
        m_guiBoard.setBackground(7,shadow);
    else
        return;
...
////////////////////////////////////////////////////////////////////////////////////////
        // modify for IGS Server (lock screen)

        else if (igsMode == true && lock == true && play == true){

        	if (! checkIgsInProcess())
                return;

        }


        else if (kgsMode == true && lock == true){
...
//// ---------------- modify this function for Remote to Engine Server. --
            ...
            if(remoteMode == true){
                ...
                try{
                    ...
                    OutputStream out = remoteserver.getOutputStream();
                    gtp = new GtpClient(in, out, m_verbose, ioCallback);
                }
                catch(IOException e){}
...
//// -------------------- New function for Print input(move from server) to board.

    public void inputMove(String in){
        ...
        try
        {
            ...
            else
            {
                ...
                if (point == null && ! isComputerBoth())
                {
                    ...
                    String mainMessage = "Pass.";
                    String optionalMessage ="Your competitor has pass.";
                    showInfo(mainMessage, optionalMessage, false);
                }
...
//// ----------- Modify for IGS server --

                if(igsMode == true){

                    String moveIgs;
                    moveIgs = GoPoint.toString(move.getPoint());
                    m_gameInfoPanel.move(moveIgs);
                }
...
//// ---------- Modify for IGS Server --
        ...
        else if(igsMode == true){

			String moveIgs;
        	moveIgs = GoPoint.toString(move.getPoint());
        	m_gameInfoPanel.move(moveIgs);
        	if(play == true){
        		showStatus("wait for commpetitor.");
        		lock = true;
        		protectGui();
        	}
        }
...
//// --------- modify function for IGS server --   	
...
//// ---------------- insert 28.07   
    ...
    public void igsRoom(String a){

    	IgsRoomFrame igsframe = new IgsRoomFrame(this,"Room list.",a);   	
    	igsframe.setSize(750,1000);
    }

    public void setTime(int times,int byoyomi,int byoyomiMoves){

    	long min = 60000;
        // 1000 = 1s, 60000 = 1m
    	TimeSettings timeSettings = new TimeSettings(times*min,byoyomi*min,byoyomiMoves);
        m_game.setTimeSettings(timeSettings);
        m_timeSettings = timeSettings;
        // try {
        //         Thread.sleep(1000);
    ...
    /* modify attachProgram to new function attachRemoteProgram() */
    private boolean attachRemoteProgram()
    {
...
// -----------   logic Remote 
            ...
            try{
                ...
                InputStream in = remoteserver.getInputStream();
                OutputStream out = remoteserver.getOutputStream();
                GtpClient gtp = new GtpClient(in, out, m_verbose, ioCallback);
...
class RemoteServer{
    ...
    public RemoteServer() throws UnknownHostException, IOException{
...
class EasyThread extends Thread {
    ...
    public void commu()throws Exception{

        int clientNumber = 0;
        ServerSocket listener = new ServerSocket(9898);
        Capitalizer capitalization = new Capitalizer(listener.accept(), clientNumber++);
...
class Capitalizer extends Thread {
        ...
        if(move.contains("IGS Server START")){

            igsMode = true;
            System.out.println("check igs = "+igsMode);

        }

    	if(igsMode == true){

    			if(move.contains("let's play naja")){


    				play = true;
    				meColor = IgsServer.getGoColor();
    				newGame(19);
    				setTime(10,10,24);


    				if(meColor == "b"){
    					System.out.println("Color : "+meColor);
    					lock = false;
    					unprotectGui();
    				}

    			}
    			else if(move.contains("this is a room")){
            		igsRoom(move);
            	}

            	if(play == true){

                        if(meColor == "w" && move.contains("play b Pass") ){
                            String mainMessage = "Pass.";
                            String optionalMessage ="Your competitor has pass.";
                            showInfo(mainMessage, optionalMessage, false);
                        }
                        else if(meColor == "b" && move.contains("play w Pass") ){
                            String mainMessage = "Pass.";
                            String optionalMessage ="Your competitor has pass.";
                            showInfo(mainMessage, optionalMessage, false);
                        }
            			else if(move.contains("play")){
            				
            	    		String result = move.substring(7,move.length());
            	    		inputMove(result);
            	    		lock = false;
            	    		unprotectGui();
            			}
                        else if(move.contains("resigned the game")){

                            String mainMessage = format(i18n("MSG_RESIGN"), "Competitor");
                            String optionalMessage =
                            "Your competitor has resigned the game.";
                            showInfo(mainMessage, optionalMessage, false);
                            lock = false;
                            unprotectGui();
                            play = false;
        ...
        else if(kgsMode == true){
            if(move.contains("play b")){
            	String result = move.substring(7,move.length());
            	inputMove(result);

            }
            else if(move.contains("play w")){
                String result = move.substring(7,move.length());
                inputMove(result);
	...
	private boolean checkIgsInProcess()
    {
        if (true)
        {
            showError("Wait",
                      "Wait for competitor thinking...", false);
            return false;
        }
        return true;
    }
